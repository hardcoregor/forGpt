...
import 'hardhat/console.sol'; //TODO: CLEAR IN ALL CONTRACTS
...
contract gammaUWYeller is Ownable {
    ...
    uint256 public previousAccShares;
    ...
    function getPendingRewards() internal view returns(uint256){
        ...
        uint wholeRewards = pendingNative + amountsOutNative[1];
        uint[] memory amountsOutYel = AggregatorV3Interface(unirouter).getAmountsOut(wholeRewards, nativeToYel);

        return amountsOutYel[1];
    }
    ...
    function getFullApr() public view returns (uint, uint) {
        ...
        uint nativeUsdtPrice = (uint(price) / divisor) * 1e18;
    ...
    function getSimpleApr(uint _nativeUsdtPrice) private view returns (uint, uint) {
        ...
        uint usdtRewardPerYear = (rewardsPerYearForPool * _nativeUsdtPrice);
    ...
    function getRewardsPerSecond() private view returns (uint) {
        ...
        Chef.PoolInfoChef memory ourPoolInfo = Chef(chef).poolInfo(poolId);
        uint lastHarvest = ourPoolInfo.lastRewardTime;

        uint rewardsPerSecond = pendingRewards / (block.timestamp - lastHarvest);

        return rewardsPerSecond;
    }

    function getLpValue() private view returns (uint, uint) {
        address lpToken = strategy.want();
        (, int price, , , ) = priceFeedWeth.latestRoundData();
        uint decimalsPriceFeed = priceFeedWeth.decimals();
        uint divisor = 10**decimalsPriceFeed;
        uint wethUsdtPrice = (uint(price) / divisor) * 1e18;

        (uint total0, uint total1) = LPToken(lpToken).getTotalAmounts();
        uint stableInLP = total0 * 1e18 / 1e6;
        uint wethEquivalentStable = total1 * wethUsdtPrice;
